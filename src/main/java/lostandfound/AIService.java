...
public class AIService {
    ...
    private static final Duration SEARCH_TIMEOUT = Duration.ofMinutes(3);
    ...
    public static String analyzeImage(String base64Image) throws Exception {
        ...
        """.formatted(AppConfig.ollamaModel, prompt, base64, AppConfig.temperature);

        String response = post(AppConfig.ollamaUrl + "/api/generate", body, IMAGE_TIMEOUT);
    ...
    public static SearchResult searchWithAI(String query, List<LostItem> allItems) throws Exception {
        ...
        String prompt = "You are a strict Lost and Found search assistant for a school.\\n\\n" +
            "Search query: \\\"" + escJson(query) + "\\\"\\n\\n" +
            "Inventory:\\n" + escJson(catalogue) + "\\n\\n" +
            "Rules:\\n" +
            "1. ONLY match items where the item TYPE matches the search query. " +
            "A phone is NOT a match for 'bag'. Eggs are NOT a match for 'phone'.\\n" +
            "2. DO NOT match based on color alone. Color can only help choose between " +
            "items that already match on type.\\n" +
            "3. If the search query is for something that does not exist in the inventory, " +
            "return an EMPTY matches array. Do not stretch or guess.\\n" +
            "4. Only return IDs of items that are a genuine match for what the person is looking for.\\n" +
            "5. Return ONLY this JSON, no markdown:\\n" +
            "{\\\"matches\\\": [\\\"ID1\\\", \\\"ID2\\\"], \\\"message\\\": \\\"explanation\\\"}\\n" +
            "Use the exact ID values as shown after 'ID:' in the inventory.";
        ...
        """.formatted(AppConfig.ollamaModel, prompt, AppConfig.temperature);

        String response = post(AppConfig.ollamaUrl + "/api/generate", body, SEARCH_TIMEOUT);
    ...
    private static String post(String url, String body, Duration timeout) throws Exception {
        ...
        if (arrStart >= 0) {
            ...
            if (bracket >= 0 && bracketEnd >= 0) {
                ...
                while (pos < arrContent.length()) {
                    ...
                    // Model sometimes echoes back the "ID:" prefix from the catalogue — strip it
